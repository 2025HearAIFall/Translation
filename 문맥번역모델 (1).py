...
# Reinstall datasets and transformers to ensure compatibility with the fixed numpy version
...
!pip uninstall -y numpy pyarrow pandas
!pip install --no-cache-dir "numpy>=2,<2.3" "pyarrow>=15,<19" "pandas>=2.2,<3" -U
...
!pip install --no-cache-dir -U "accelerate>=0.26.1" "transformers>=4.47,<4.58"
#런타임 재시작 후 다시 실행 그래야 밑에코드 실행된다
...
training_args = Seq2SeqTrainingArguments(
    ...
    num_train_epochs=4,              # 3~5 사이에서 시작 권장
    weight_decay=0.01,
...
trainer = Seq2SeqTrainer(
    model=model,
    args=training_args,
    tokenizer=tokenizer, # Changed processing_class to tokenizer and used the tokenizer object
